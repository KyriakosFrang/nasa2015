...
VALUES(?,?,?,?,?,?,?,?,?,?,?,?)");
		...
		//calculate results
									...
									$sql = "SELECT Temperature_thressholds.name,Temperature_thressholds.id_thresshold FROM Temperature_thressholds WHERE Temperature_thressholds.min_calc<='".$obj->curr_temp."' AND Temperature_thressholds.max_calc>='".$obj->curr_temp."';";
									$result = $conn->query($sql);
									...
									$row = $result->fetch_assoc();
	 								...
	 								$result->close();
										...
										$temp_thress_name='-1';
												...
												//airflow thresshold
									$sql = "SELECT Airflow_thressholds.name,Airflow_thressholds.id_thresshold FROM Airflow_thressholds WHERE Airflow_thressholds.min_calc<='".$obj->curr_airflow."' AND Airflow_thressholds.max_calc>='".$obj->curr_airflow."';";
									$result = $conn->query($sql);
									...
									$row = $result->fetch_assoc();
										...
										$airflow_thress_name=$row['name'];
	 								$result->close();
									}
									 else {
										
    									$airflow_thress_id='-1';
										$airflow_thress_name='-1';
					...
					//blood oxygen thresshold
									$sql = "SELECT Blood_Oxygen_thressholds.name,Blood_Oxygen_thressholds.id_thresshold FROM Blood_Oxygen_thressholds WHERE Blood_Oxygen_thressholds.min_calc<='".$obj->curr_blood_oxyg."' AND Blood_Oxygen_thressholds.max_calc>='".$obj->curr_blood_oxyg."';";
									$result = $conn->query($sql);
									...
									$row = $result->fetch_assoc();
										...
										$blood_oxygen_thress_name=$row['name'];
		...
		//systolic pressure thresshold
									$sql = "SELECT Systolic_Pressure_thressholds.name,Systolic_Pressure_thressholds.id_thresshold FROM Systolic_Pressure_thressholds WHERE Systolic_Pressure_thressholds.min_calc<='".$obj->curr_systolic_press."' AND Systolic_Pressure_thressholds.max_calc>='".$obj->curr_systolic_press."';";
									$result = $conn->query($sql);
									...
									$row = $result->fetch_assoc();
										...
										$systolic_press_thress_name=$row['name'];
		...
		//diastolic pressure thresshold
									$sql = "SELECT Diastolic_Pressure_thressholds.name,Diastolic_Pressure_thressholds.id_thresshold FROM Diastolic_Pressure_thressholds WHERE Diastolic_Pressure_thressholds.min_calc<='".$obj->curr_diastolic_press."' AND Diastolic_Pressure_thressholds.max_calc>='".$obj->curr_diastolic_press."';";
									$result = $conn->query($sql);
									...
									$row = $result->fetch_assoc();
										...
										$diastolic_press_thress_name=$row['name'];
	 								$result->close();
									}
									 else {
										
    									$diastolic_press_thress_id='-1';
										$diastolic_press_thress_name='-1';
									}
									...
									$sql = "SELECT Muscle_Intensity_thressholds.name,Muscle_Intensity_thressholds.id_thresshold FROM Muscle_Intensity_thressholds WHERE Muscle_Intensity_thressholds.min_calc<='".$obj->curr_muscl_intens."' AND Muscle_Intensity_thressholds.max_calc>='".$obj->curr_muscl_intens."';";
									$result = $conn->query($sql);
									...
									$row = $result->fetch_assoc();
										...
										$muscle_intensity_thress_name=$row['name'];
	 								$result->close();
									}
									 else {
										
    									$muscle_intensity_thress_id='-1';
										$muscle_intensity_thress_name='-1';
									...
									//ECG thresshold
									$sql = "SELECT ECG_thressholds.name,ECG_thressholds.id_thresshold FROM ECG_thressholds WHERE ECG_thressholds.min_calc<='".$obj->curr_ecg."' AND ECG_thressholds.max_calc>='".$obj->curr_ecg."';";
									$result = $conn->query($sql);
									...
									$sql = "SELECT Contactivity_thressholds.name,Contactivity_thressholds.id_thresshold FROM Contactivity_thressholds WHERE Contactivity_thressholds.min_calc<='".$obj->curr_contactivity."' AND Contactivity_thressholds.max_calc>='".$obj->curr_contactivity."';";
									$result = $conn->query($sql);
									...
									$row = $result->fetch_assoc();
										...
										$conductivity_thress_name=$row['name'];
	 								$result->close();
									}
									 else {
										
    									$conductivity_thress_id='-1';
										$conductivity_thress_name='-1';
									}
		...
		//Pulse thresshold
									$sql = "SELECT Pulse_thressholds.name,Pulse_thressholds.id_thresshold FROM Pulse_thressholds WHERE Pulse_thressholds.min_calc<='".$obj->curr_pulse."' AND Pulse_thressholds.max_calc>='".$obj->curr_pulse."';";
									$result = $conn->query($sql);
		...
		//Radiation thresshold
									$sql = "SELECT Radiation_thressholds.name,Radiation_thressholds.id_thresshold FROM Radiation_thressholds WHERE Radiation_thressholds.min_calc<='".$obj->curr_radiation."' AND Radiation_thressholds.max_calc>='".$obj->curr_radiation."';";
									$result = $conn->query($sql);
									...
									$row = $result->fetch_assoc();
										...
										$radiation_thress_name=$row['name'];
	 								$result->close();
									}
									 else {
										
    									$radiation_thress_id='-1';
										$radiation_thress_name='-1';
									}
									...
									$sql = "SELECT User.Username,Current_State.Name, User.Name_Surname,User.idUser FROM User INNER JOIN Journey_has_User ON Journey_has_User.User_idUser=User.idUser INNER JOIN Login ON Journey_has_User.Journey_idJourney=Login.Journey_has_User_Journey_idJourney AND Journey_has_User.User_idUser=Login.Journey_has_User_User_idUser AND Journey_has_User.Start_date=Login.Journey_has_User_Start_date INNER JOIN Current_State ON Current_State.idCurrent_State=Login.Current_State_idCurrent_State WHERE Login.idLogin='".$obj->login_id."';";
									$result = $conn->query($sql);
									...
									$row = $result->fetch_assoc();
	 								...
	 								$result->close();
										...
										$sql = "SELECT Journey.name FROM Journey_has_User INNER JOIN Journey ON Journey_has_User.Journey_idJourney=Journey.idJourney WHERE ISNULL(Journey_has_User.End_date) AND Journey_has_User.User_idUser='".$idhuman."';";
									$result = $conn->query($sql);
...
VALUES(?,?,?,?,?,?,?,?,?,?,?,?)");
									...
									echo json_encode($a."_".$b."_".$c."_".$d."_".$e."_".$f."_".$g."_".$h."_".$i."_".$j."_".$k."_".$l."####".$temp_thress_name."_".$airflow_thress_name."_".$blood_oxygen_thress_name."_".$systolic_press_thress_name."_".$diastolic_press_thress_name."_".$muscle_intensity_thress_name."_".$ecg_thress_name."_".$conductivity_thress_name."_".$pulse_thress_name."_".$radiation_thress_name."####".$username."####".$statename."####".$namesurname."####".$journeyname);
